...
class CMGMM(GaussianMixture):
	...
	def __init__(self,min_components=3, max_components=10,distance_method="Kullback-leiber",merge_type="moment_presaving"):
	...
	def fit(self, data):
		...
		if (self.initialized==False):			
			...
			'''
			w,m,c = self.mixture_reduction(w_all, mu_all, cov_all, 8, isomorphic=False, verbose=False, optimization=False)
			self.weights_ = w
			self.means_ = m
			self.covariances_= c
			self.n_components = len(w)
			#Compute the Cholesky decomposition of the precisions.
			self.precisions_cholesky_ = _compute_precision_cholesky(c, self.covariance_type)
			return 1;
			'''
			#n_comp =5
			#print(w_all.shape)
			n_components_range =  range(w_all.shape[0], self.n_components, -1)
		
			bicreduced=[]
			lowest_bic = np.infty
			jumlahSample = 1000*len(data)
			#if (jumlahSample<=100):
			#	jumlahSample=1000
			...
			for n_components in n_components_range:
				w,m,c = self.mixture_reduction(w_all, mu_all, cov_all, n_components, isomorphic=False, verbose=False)
