...
class KD3:
    ...
    def integrate_distance(self, kde1,kde2,bound):
        chk1,e = integrate.quad(lambda x: np.exp(kde1.score_samples(np.array([x]).reshape(-1,1)))[0], bound[0], bound[1])
        chk2,e = integrate.quad(lambda x: np.exp(kde2.score_samples(np.array([x]).reshape(-1,1)))[0], bound[0], bound[1])
        diff = chk1 - chk2
        # diff = diff * diff
